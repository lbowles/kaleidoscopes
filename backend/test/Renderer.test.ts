...
describe("Renderer", function () {
  ...
  let tokenId: number
  let kaleidoscope: Renderer.KaleidoscopeStructOutput
  ...
  beforeEach(async function () {
    ...
    kaleidoscope = await renderer.kaleidoscopeForTokenId(tokenId)
    colorPalette = await renderer.colorPaletteForKaleidescope(kaleidoscope)
  })

  // it("Should render circles properly", async () => {
  //   console.log(kaleidoscope)
  //   const circle = await renderer.circleAtIndexForKaleidescope(kaleidoscope, colorPalette, 1)
  //   console.log(circle)
